    ...
    function observe() external view override returns (uint256) {
        int24 tick =
            OracleLibrary.consult(
                PoolAddress.computeAddress(uniswapV3Factory, PoolAddress.getPoolKey(_token, wethAddress, 500)),
                _period
            );
        uint256 weth = OracleLibrary.getQuoteAtTick(tick, 1 ether, _token, wethAddress);
        tick = OracleLibrary.consult(
            PoolAddress.computeAddress(uniswapV3Factory, PoolAddress.getPoolKey(wethAddress, usdcAddress, 3000)),
            _period
        );
        // TODO: Safe cast here
